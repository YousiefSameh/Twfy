...
class CssGenerator {
    generate(theme, options = {}) {
        ...
        if (minify) {
            ...
            if (darkCss)
                darkCss = (0, utils_1.minifyCss)(darkCss);
            if (animationsCss)
                animationsCss = (0, utils_1.minifyCss)(animationsCss);
        }
        return {
            css: mainCss,
    ...
    generateMainCss(theme) {
        ...
        for (const [name, config] of Object.entries(theme.fontSizes)) {
            css += (0, utils_1.formatCssVariable)(`font-size-${name}`, config.size) + '\n';
            if (config.lineHeight) {
                css += (0, utils_1.formatCssVariable)(`line-height-${name}`, config.lineHeight) + '\n';
            }
        }
        // Spacing
        ...
        for (const [category, properties] of Object.entries(theme.custom)) {
            for (const [name, value] of Object.entries(properties)) {
                css += (0, utils_1.formatCssVariable)(`${category}-${name}`, value) + '\n';
            }
        }
        css += '}\n';
        return css;
    ...
    generateDarkCss(theme) {
        ...
        const darkColors = Object.entries(theme.colors).filter(([name]) => name.includes('-dark-') || name.endsWith('-dark'));
    ...
    generateAnimationsCss(theme) {
        let css = (0, utils_1.generateHeader)('Twfy Tokens - Animations & Keyframes');
        // Keyframes
        ...
        if (Object.keys(theme.animations).length > 0) {
            ...
            for (const [name, animation] of Object.entries(theme.animations)) {
                css += '  ' + (0, utils_1.formatAnimationClass)(name, animation).replace(/\n/g, '\n  ') + '\n\n';
            }
            css += '}\n';
        }
        return css;
    ...
    generateReport(theme) {
        const totalConverted = Object.keys(theme.colors).length +
